...
import { validateBank, validateField } from "./validation";
import Columns from "./columns";
import useFetchData from "common/customHooks/useFetchData";
import UpsertBank from "./upsertBank";
...
const ViewBank = () => {
  ...
  const [isBank, setIsBank] = useState(initialValues);
  ...
  const saveData = async () => {
    const newData = { ...isBank };

    if (isBank.receivedOrSent === "Own") {
      newData.partyName = "";
    ...
    const errors = validateBank(isBank);
    setValidationErrors(errors);

    // Check if there are no errors before proceeding
    if (Object.keys(errors).length === 0) {
      const updatedData = { assetType: "Bank account", ...newData };
