...
import { validateField, validateProperties } from "./validation";
import Columns from "./columns";
import useFetchData from "common/customHooks/useFetchData";
import UpsertProperties from "./upsertProperties";
...
const ViewProperties = () => {
  ...
  const [isProperties, setIsProperties] = useState(initialValues);
  ...
  const saveData = async () => {
    const newData = { ...isProperties };

    if (isProperties.receivedOrSent === "Own") {
      newData.partyName = "";
    ...
    const errors = validateProperties(isProperties);
    setValidationErrors(errors);

    // Check if there are no errors before proceeding
    if (Object.keys(errors).length === 0) {
      const updatedData = { assetType: "Properties", ...newData };
